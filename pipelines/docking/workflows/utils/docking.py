...
def plot_docking_scatter_with_errorbars(csv_path: Path, output_dir: Path):
    ...
    # reference lines
    plt.axhline(-7.0, linestyle="--", color="red", alpha=0.7, label="Low")
    plt.axhline(-8.0, linestyle="--", color="orange", alpha=0.7, label="Medium")
    plt.axhline(-9.0, linestyle="--", color="green", alpha=0.7, label="High")

    plt.xlabel("Molecule (ligand index)")
    plt.ylabel("Score (kcal/mol) - GNINA")
    plt.title("Docking score range for all poses x ligand")
