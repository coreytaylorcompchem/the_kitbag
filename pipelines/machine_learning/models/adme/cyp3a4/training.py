...
def weighted_mse(preds, targets, bin_edges, bin_counts, min_w, max_w):
    y = targets.detach().cpu().numpy()
    bin_ids = np.digitize(y, bin_edges[:-1], right=True) - 1
    bin_ids = np.clip(bin_ids, 0, len(bin_counts) - 1)

    weights = 1.0 / bin_counts[bin_ids]
    weights = weights / np.mean(weights)
    weights = np.clip(weights, min_w, max_w)

    weights = torch.tensor(weights, device=preds.device, dtype=torch.float32)
...
def train(context, config):
    ...
    if loss_type == "weighted_mse":
        train_labels = np.array([
            g.y.item()
            for g in train_list
            if g.y is not None and torch.isfinite(g.y)
        ])

        if len(train_labels) == 0:
            raise ValueError("No finite labels found for weighted loss")

        hist, bin_edges = np.histogram(
            train_labels,
            bins=loss_cfg.get("n_bins", 50)
        )
        bin_counts = np.maximum(hist, 1)

        min_w = loss_cfg.get("min_weight", 0.2)
        max_w = loss_cfg.get("max_weight", 5.0)
        ...
        logger.info(
            f"Using batch-weighted MSE "
            f"(bins={len(hist)}, min_w={min_w}, max_w={max_w})"
        )
    else:
    ...
    for lr, hidden_dim in itertools.product(
        ...
        for epoch in trange(
            ...
            desc=f"Training lr={lr}, hidden={hidden_dim}",
        ):
        ...
        if final_val_loss < best_loss:
            ...
            logger.info(
                f"Saved best model (val_loss={best_loss:.4f}) to {model_path}"
            )
